...
const deleteOrder = async () => {
    ...
    try {
        await apiInstance.delete(`/user/orders/${props.orderedItem.id}`);
        toast.success(`成功取消訂單 #${props.orderedItem.id}`);
        try {
    ...
    <div class="row mb-3">
        ...
        <div class="col-12 col-md-2">
            <div class="d-flex justify-content-end align-content-center">
                <button type="button" class="btn btn-sm btn-danger" @click="deleteOrderState.execute" :disabled="deleteOrderState.isLoading.value || orderedItem.status != 1">取消訂單</button>
            </div>
        </div>
    </div>
</template>